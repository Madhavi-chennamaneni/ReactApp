    ...
    return (
            <div className='workPath'>
                <button className='workToDoBtn'>
                    Work To Do&nbsp;
                    <FontAwesomeIcon className='arrowIcon' icon={(isOpen === true) ? faChevronDown : faChevronUp} onClick={() => setIsOpen(!isOpen)} />
                </button>
                {(isOpen === true) ? (
                    <>
                        <div className='overDueSection'> 
                            <div className='DueList'>
                                 <OverDue datas={assignmentDatas} currentWeek={currentWeek}  FontAwesomeIcon={FontAwesomeIcon} faFileLines={faFileLines}/>
                                <div className=''>
                                    <h3 className='nextWeekDate'>{firstday} - {lastday}</h3><hr />
                                    <WeekDue datas={assignmentDatas} nextWeek={nextWeek} 
